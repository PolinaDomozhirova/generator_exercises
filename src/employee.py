...
# декоратор, чтобы не было ошибки...метод статический тк нет селф в арг методв
    ...
    def _count_salary(self, currency):
        if self.__salary and currency == self.__salary.currency:
            ...
            if currency == CURRENCIES.RUBLE and self.__salary.currency == CURRENCIES.DOLLAR:
                return self.__salary.amount * 88
            elif currency == CURRENCIES.DOLLAR and self.__salary.currency == CURRENCIES.RUBLE:
                return self.__salary.amount / 88
            else:
                raise AttributeError("Invalid currency conversion")
